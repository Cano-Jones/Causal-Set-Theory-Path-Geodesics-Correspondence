...
This example script shows the path-geodesic correspondence in a Minkonskian background for a timelike trajectory.
"""
...
if __name__ == "__main__":
    ...
    g = MetricTensor(kappa=0, a=lambda t: 1) # No curvature, no expansion

    #Here we specify the simulation parameters 
    simCauset = CausetSimulation(Metric=g, TimeRange=(0,2), SpaceRange=(-1,1), PointNumber=150, Divisions=(100,100))

    #For the continuum geodesic we employ the following code
    simContinuum = ContinuumSimulation(Metric=g, source=(0,0), SpacialVelocity=0.5, tau_span=linspace(0,100,1000), g_type='timelike')
